...
APP="$DERIVED_DATA/Build/Products/Release/MySQLMacClient.app"
...
# A previous DMG (this one or an older version) left mounted under
...
echo "==> Ejecting any mounted MySQLMacClient volumes"
for mount in /Volumes/MySQLMacClient*; do
    [ -d "$mount" ] || continue
    hdiutil detach "$mount" -quiet 2>/dev/null || hdiutil detach "$mount" -force -quiet 2>/dev/null || true
done

echo "==> Creating DMG"
hdiutil create -volname "MySQLMacClient" -srcfolder "$STAGE" -ov -format UDZO "$DMG_PATH"
